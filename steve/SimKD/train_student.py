...
def parse_option():
    ...
    parser.add_argument(
        "-c", "--cls", type=float, default=1.0, help="weight for classification"
    )
    parser.add_argument(
        "-d", "--div", type=float, default=1.0, help="weight balance for KD"
    )
    parser.add_argument(
        "-b", "--beta", type=float, default=0.0, help="weight balance for other losses"
    )
    parser.add_argument(
        "-f", "--factor", type=int, default=2, help="factor size of SimKD"
    )
    parser.add_argument(
        "-s", "--soft", type=float, default=1.0, help="attention scale of SemCKD"
    )
    ...
    parser.add_argument(
        "--dist-url",
        default="tcp://127.0.0.1:23451",
        type=str,
        help="url used to set up distributed training",
    )
    parser.add_argument(
        "--deterministic", action="store_true", help="Make results reproducible"
    )
    parser.add_argument(
    ...
    model_name_template = split_symbol.join(
        [
            "S",
            "{}_T",
    ...
    opt.model_name = model_name_template.format(
        opt.model_s,
        opt.model_t,
        opt.dataset,
        opt.distill,
...
def load_teacher(model_path, n_cls, gpu=None, opt=None):
    ...
    map_location = (
        None
        if gpu is None
        else {"cuda:0": "cuda:%d" % (gpu if opt.multiprocessing_distributed else 0)}
    )
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    if opt.dataset == "cifar100":
        ...
        if opt.dali is None:
            if opt.distill in ["crd"]:
                train_loader, val_loader, n_data, _, train_sampler = (
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        # print('++++++++++++++++++++++++++++++++++++++++')
        # print(module_list[0])
        # for name, param in module_list[0].named_parameters():
        #     if param.requires_grad:
        #         print(name, param.data)
        # summary(module_list[0], (3, 32, 32))
        # break
        # mobilenetV2x2 =  '2,353,060'
        # resnet32x4 = '7,433,860'
